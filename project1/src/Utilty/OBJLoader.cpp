             ...
             std::string &texture)
{
    FILE * file = fopen(path, "r");
    if( file == NULL ){
        printf("Impossible to open the .mtl file! Path: %s\n", path);
        return false;
    }
    ...
    while( 1 ){
        ...
        if(strcmp(lineHeader , "newmtl") == 0) {
            ...
            // 預設值佔位，確保所有 vector 同步增長
            Ka.push_back(vec3(1.0f, 1.0f, 1.0f));
            Kd.push_back(vec3(0.6f, 0.6f, 0.6f));
            Ks.push_back(vec3(0.5f, 0.5f, 0.5f));
            Ke.push_back(vec3(0.0f, 0.0f, 0.0f));
            Ni.push_back(1.0f);
            Ns.push_back(250.0f);
            d.push_back(1.0f);
            illum.push_back(2);
            ...
            fscanf(file, "%f %f %f\n", &Kd.back().x, &Kd.back().y, &Kd.back().z);
        }
        else if(strcmp(lineHeader, "Ka") == 0) {
            fscanf(file, "%f %f %f\n", &Ka.back().x, &Ka.back().y, &Ka.back().z);
        }
        else if(strcmp(lineHeader, "Ks") == 0) {
            fscanf(file, "%f %f %f\n", &Ks.back().x, &Ks.back().y, &Ks.back().z);
        }
        else if (strcmp(lineHeader, "Ke") == 0) {
            fscanf(file, "%f %f %f\n", &Ke.back().x, &Ke.back().y, &Ke.back().z);
        }
